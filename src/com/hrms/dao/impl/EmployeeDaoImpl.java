...
public class EmployeeDaoImpl implements IEmployeeDao {
	...
	public int findEmployeeSalary( String low,String high) {
		basedao = new BaseDao();
		List<Object> params = null;
		//新建List<Employee>，存放Employee的元素
		List<Employee> employeeList = new ArrayList<Employee>();
		//在某个时间段的查询sql语句
		String sql = "select count(*) from employees group by salary having salary > "+ low + " and salary <= " +high;
		//查询的返回结果集
		ResultSet result = basedao.executeQuery(sql, params);
		int length = 0;
		try {
			result.last();
			length = result.getRow();
		} catch (SQLException e) {
			System.err.println("取长度错误，置0");
		}
		//关闭basedao
		basedao.closeConnection();
		return length;
